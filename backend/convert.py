...
def excel_to_sqlite(folder_path, db_name):
    ...
    if not os.path.isdir(folder_path):
        print(f"Ошибка: Папка {folder_path} не найдена. Создайте её и положите туда 3 файла.")
        return

    # Получаем список всех Excel файлов в папке
    files_to_process = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith(('.xlsx', '.xls'))]
    
    print(f"Найдено файлов для обработки: {len(files_to_process)}")

    for file_path in files_to_process:
        # Имя таблицы делаем строго по имени файла (без расширения)
        file_name_clean = os.path.splitext(os.path.basename(file_path))[0]
        table_name = file_name_clean.strip().replace(" ", "_").replace("-", "_")
        ...
        print(f"  -> Таблица '{table_name}' успешно создана/обновлена. Всего строк: {total_rows}")
